        ...
        <div>
          <table cellSpacing="10">
            ...
            {this.props.product.map((data,index) => {
              return (
                  <tr>
                    <td><button onClick={()=>this.handleDecrese(index,data.category,data.price)}>-</button><button onClick={()=>this.handleIncrese(index,data.category,data.price)}>+</button><br/>{data.name}({data.quantity})</td>
                    <td>&#8377;{data.price}</td>
                    <td>&#8377;{data.totalPrice}</td>
                    <td><Icon className={classes.icon} onClick={()=>this.handlDelete(index,data.category)}>remove_shopping_cart</Icon></td>
                  </tr>
              );
            })}
        </table>
